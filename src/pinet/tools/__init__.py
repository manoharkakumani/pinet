"""Tools package for Pinet Agents"""
from importlib import import_module
from typing import Any

from pinet.utils import load_user_tools
...
TOOL_MAPPINGS = {
    ...
    # 'kg_tools': ('.kg_tools', 'KnowledgeGraphMemory'),
    'add_fact': ('.kg_tools', None),
    'query_kg': ('.kg_tools', None),
    'reset_kg': ('.kg_tools', None),
    'get_all_facts': ('.kg_tools', None),
    'visualize_kg': ('.kg_tools', None),

    # Voice Tools
    'voice_chat_tool': ('.voice_chat_tool',  None),
    'start_voice': ('.voice_chat_tool',  None),
    # 'stop_voice': ('.voice_chat_tool', 'VoiceTools'),
    ...
    'cot_tools': ('.train.data.generatecot', None),                           # Full toolkit access
}
...
def __getattr__(name: str) -> Any:
    ...
    if name not in TOOL_MAPPINGS and name not in user_tools:
        raise AttributeError(f"module '{__package__}' has no attribute '{name}'")

    if name in user_tools:
        return user_tools[name]
...
__all__ = list(TOOL_MAPPINGS.keys()) + list(user_tools.keys())